 ...
 * {@code /skyblock/garden} (garden experience + unlocked plots). All calls are
 * blocking and run from {@link PlayerDataService}'s background thread.
 */
public final class HypixelApi {
    ...
    public static PlayerInfo fetchPlayer(String apiKey, String undashedUuid) throws Exception {
        JsonObject root = HttpJson.get(BASE + "/skyblock/profiles?uuid=" + undashedUuid, apiKey);
        ...
        GardenData garden = fetchGarden(apiKey, profileId);
        return new PlayerInfo(true, ironman, profileId, cute, garden);
    }

    private static GardenData fetchGarden(String apiKey, String profileId) {
        try {
            JsonObject root = HttpJson.get(BASE + "/skyblock/garden?profile=" + profileId, apiKey);
